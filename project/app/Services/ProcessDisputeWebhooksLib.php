...
use App\Jobs\ProcessDisputeWebhooks;

class ProcessDisputeWebhooksLib
{
    ...
    public function trigger()
    {
        ...
        $response = $process_webhooks->process_freshworks_connector($this->db_channel_config, $this->event_topic, $data, 'Dispute');
        ...
        if (!$this->is_test_webhook && isset($response) && !empty($response)){
            ...
            if (!$response['status'] && $response['message'] == config('channel.code_503_message')) {
                $queue = new ProcessDisputeWebhooks($this->db_channel_config, $this->shopify_request_data, $this->shop, $this->event_topic, $this->shopify_request_header, $this->event_name);
