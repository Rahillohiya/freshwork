...
use App\Jobs\ProcessInventoryItemLevelWebhooks;
use Illuminate\Support\Str;

class ProcessInventoryItemLevelWebhooksLib
{
    ...
    {
        $is_inventory_level = Str::contains('levels', $this->event_topic);
        ...
        $response = $process_webhooks->process_freshworks_connector($this->db_channel_config, $this->event_topic, $data, ($is_inventory_level ? 'Inventory Level' : 'Inventory Item'));
        ...
        if (!$this->is_test_webhook && isset($response) && !empty($response)){
            ...
            if (!$response['status'] && $response['message'] == config('channel.code_503_message')) {
                $queue = new ProcessInventoryItemLevelWebhooks($this->db_channel_config, $this->shopify_request_data, $this->shop, $this->event_topic, $this->shopify_request_header, $this->event_name);
