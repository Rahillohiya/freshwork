...
use App\Jobs\ProcessShopAlternateLocaleWebhooks;

class ProcessShopAlternateLocaleWebhooksLib
{
    ...
    public function trigger()
    {
        ...
        if (!$this->is_test_webhook && isset($response) && !empty($response)){
            ...
            if (!$response['status'] && $response['message'] == config('channel.code_503_message')) {
                $queue = new ProcessShopAlternateLocaleWebhooks($this->db_channel_config, $this->shopify_request_data, $this->shop, $this->event_topic, $this->shopify_request_header, $this->event_name);
