...
class ProcessWebhooks
{
    ...
    */
    public function process_freshworks_connector($config, $event_topic, $shopify_webhook_data, $webhook_event = "")
    {
        ...
        try {
            $freshworks_event_settings = $config->channel_event_settings;

            if (!$freshworks_event_settings || !$channel_event = $freshworks_event_settings->channel_event)
            ...
            $freshworks_account = $freshworks_event_settings->channel_account;
            if (!$freshworks_account)
                return ['status' => $notification_status, 'message' =>
                    'No freshworks account details found.Please make sure you have not removed or updated the freshworks account settings.'
                ];

            $freshworks_api_library = new \App\Services\Connectors\FreshworksApiLibrary();

            if ($channel_event->slug == "create_contacts" || $channel_event->slug == "update_contacts") {
                $response = $freshworks_api_library->create_or_update_contacts($freshworks_account, $freshworks_event_settings, $shopify_webhook_data, $channel_event);
            }
            ...
            if ($response && !$response['status'] && $response['message'] == "Rate Limit Issue") return ['status' => $notification_status, 'message' => $response['message'], 'retry_after' => $response['retry_after'] ?? 32];
            else return ['status' => $notification_status, 'message' => $response_message];
        } catch (\Exception $e) {
