...
class ProcessOrderWebhooksLib
{
    ...
    {
        $is_draft_order = Str::contains($this->event_topic, 'draft');
        ...
        if (!$this->is_test_webhook && !empty($this->db_channel_config->execution_conditions) && !Str::contains($this->event_topic, '/delete')) {
            ...
            if ($is_draft_order) {
                ...
                // using below 3 paramters in filter conditions string i-e if ($financial_status=='any')
                $financial_status = $data['financial_status'] ?? '';
                $fulfillment_status = $data['fulfillment_status'] ?? 'unshipped';
        ...
        $response = $process_webhooks->process_freshworks_connector($this->db_channel_config, $this->event_topic, $data, ($is_draft_order ? 'Draft Order' : 'Order'));
        ...
        if (!$this->is_test_webhook && isset($response) && !empty($response)){
            ...
            if (!$response['status'] && $response['message'] == config('channel.code_503_message')) {
                $queue = new ProcessOrderWebhooks($this->db_channel_config, $this->shopify_request_data, $this->shop, $this->event_topic, $this->shopify_request_header, $this->event_name);
