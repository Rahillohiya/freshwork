...
use App\Jobs\ProcessOrderEditWebhooks;

class ProcessOrderEditWebhooksLib
{
    ...
    public function trigger()
    {
        ...
        if (!$this->is_test_webhook && !empty($this->db_channel_config->execution_conditions)) {
            $execution_conditions = $this->db_channel_config->execution_conditions;
            $created_at = date('Y-m-d', strtotime($data['order_edit']['created_at']));
            $condition_str = $ShopifyService->filter_webhook_conditions_str_format($execution_conditions);
            try {
                // run condition in php , available in string format
                $is_allowed_webhook = eval($condition_str);
            } catch (\Throwable $e) {
                $webhook_logs->save_logs(0, $this->db_channel_config, $data['order_edit'], $this->shop, $e->getMessage() . ".Please contact support team");
            }
        }
        ...
        $response = $process_webhooks->process_freshworks_connector($this->db_channel_config, $this->event_topic, ($data['order_edit']??$data),  'OrderEdit');
        ...
        if (!$this->is_test_webhook && isset($response) && !empty($response)){
            ...
            if (!$response['status'] && $response['message'] == config('channel.code_503_message')) {
                $queue = new ProcessOrderEditWebhooks($this->db_channel_config, $this->shopify_request_data, $this->shop, $this->event_topic, $this->shopify_request_header, $this->event_name);
