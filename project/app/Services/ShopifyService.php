        ...
        try {
            ...
            if ($plan_type == 'basic') {
                ...
                if ($plan_type == 'professional') {
                    $post_data = array('recurring_application_charge' => array('name' => 'Professional', 'trial_days' => 0, 'price' => config('shopify.professional_plan_price', 19), 'format' => 'json',
                        'return_url' => $return_url, 'test' => config('shopify.app_test_mode', false)));
                } else {
                    $post_data = array('recurring_application_charge' => array('name' => 'Elite', 'trial_days' => 0, 'price' => config('shopify.elite_plan_price', 29), 'format' => 'json',
                        'return_url' => $return_url, 'test' => config('shopify.app_test_mode', false)));
                }
